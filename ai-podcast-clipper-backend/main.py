...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, framerate=25):
    ...
    for fidx, fname in tqdm(enumerate(flist), total=len(flist), desc="Creating vertical video"):
        ...
        if mode == "resize":
            ...
            if image_cropped.shape[1] < target_width:
                pad_left = (target_width - image_cropped.shape[1]) // 2
                pad_right = target_width - image_cropped.shape[1] - pad_left
                image_cropped = cv2.copyMakeBorder(image_cropped, 0, 0, pad_left, pad_right, cv2.BORDER_CONSTANT, value=[0, 0, 0])
            elif image_cropped.shape[1] > target_width:
                excess_width = image_cropped.shape[1] - target_width
                crop_left = excess_width // 2
                image_cropped = image_cropped[:, crop_left:crop_left + target_width]
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(
        subtitle_output_path, "ai-podcast-clipper-avr", output_s3_key 
    )

@app.cls(
    gpu="L40S", timeout=900, retries=0, scaledown_window=20,
...
class AIPodcastClipper:
    ...
    def process_video(self,request: ProcessVideoRequest,token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-avr",s3_key, str(video_path))
    ...
    def main():
        ...
        payload = {"s3_key": "test1/Andrew5min.mp4"}
        headers = {
